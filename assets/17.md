...
第一に`["add", k, mv]`のケースでは、ユーザーが入力したコマンドにおける、3つめの単語`mv`を`readMaybe`関数で数値に変換しています。`add`コマンドが入力されたら入力した数値を足し算する、という要件があるからですね。そして、`readMaybe`が`Just`を返したら --- すなわち数値への変換が成功したら ---、
