...
という行では、`NotFound`という値コンストラクターにマッチさせていますが、`NotFound`には何も値が含まれていないため、特に変数に代入させてはいません。なのでそのまま`putStrLn "Unknown"`と実行しています（気が向いたら`lifeUniverseAndEverything`に`NotFound`を代入して先ほどの`case`式を再度試してみてください！）。
