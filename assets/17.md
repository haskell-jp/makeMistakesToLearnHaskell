...
`main`関数を一行ずつ見ていきましょう。最初に、プロンプトとして`"Enter command:"`という文字列を出力した後、`getLine`関数でユーザーが入力した文字列を取得します。そして入力した文字列`command`を`words`関数で分解する --- 問題はその次の`case`式です。`["add", k, mv]`、`["get", k]`などといった具合に、リストの各要素が具体的な値になっていることをチェックしています。このようなパターンマッチングはこれまでの課題では使うことがなかったため、少し掘り下げましょう。
