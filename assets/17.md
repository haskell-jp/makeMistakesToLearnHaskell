...
前節までが、「連想配列を更新するロジック」を除いて処理を繰り返す部分のみの解説です。ここからはこれに、「更新するロジック」を加えます。ただし、そのまま課題の仕様の通り「連想配列を更新するロジック」を紹介してしまうとやりがいがなくなってしまうので、代わりに整数を一つだけ`add`したり`get`したりするプログラムにしましょう。ユーザーが入力したキーは無視することにします。

課題hoge(12?)の`concatLines`名前の関数で例示した通り、関数を再帰呼び出ししつつ呼び出す度に状態を更新する、というような処理が必要な場合、それまで対象の関数で参照・更新してきた値（即ち状態）を、対象の関数を再帰呼び出しする際、新しい引数として渡す必要があります。更新するときは引数を変える、というよくあるパターンです。
