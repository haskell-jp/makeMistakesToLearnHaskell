...
`IO`に対して`<$>`や`<*>`が使えるように、`Maybe`に対しても`<$>`や`<*>`が使えることを、これまでこの課題では学びました。それと同様に、`>>=`や`>>`も`Maybe`に対して使えるようになっています。そして`>>=`や`>>`が使えるということは、それらの糖衣構文である、`do`記法も使うことができる、ということを意味します。`Maybe`における`>>=`や`>>`とは、一体どのように振る舞うのでしょうか？ここでは、よく使う上に今回の課題を解くのにも必要な`>>=`と、`>>=`を`do`記法の中で使ったバージョンの`<-`について解説します（`>>`については、是非自分で試してみてください！）。
