...
ここまで`do`などの役割を説明するにあたり、主に`IO`を使用してきました。`IO`はこれまで何度もカッコ書きしてきたとおり（入出力ができる）「命令」ですから、他の「命令っぽいもの」についても、ある程度類推して理解できると私は信じているからです。  
...
（注意: この数え方に明確な基準があるわけではないです。ましてや「`Maybe`における『命令』の種類は1種類だ」なんてことはどこでも合意されてません。あくまでも「`IO`に比べて`Maybe`は`do`の中でできることが限られてる」ということを伝えるための便宜だと思ってください）
...
例えば、下記のように`Maybe`に対する`do`を書いているところに`putStrLn`などの`IO`型の命令を混ぜることはできません。
...
そのようなケースの解決策として、[`MaybeT`][5]という型があります。
...
下記のように書けば、`MaybeT IO`という「入出力処理を実行した後、結果が`Just`か`Nothing`かをチェックする」命令を持った`Monad`型クラスのインスタンスの型が生まれます。
...
つまり、`MaybeT IO`の`<*>`は、左辺に渡した`MaybeT IO (a -> b)`の命令を実行して結果が`Nothing`だったら右辺の`MaybeT IO a`の`IO`の命令から実行しない、という実装になっているのです。便利。
