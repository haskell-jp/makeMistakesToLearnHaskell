...
`(+)`は`Num`型クラスのインスタンスである型全般に対して使えるので、`No instance for (Num (Maybe Integer)) arising from a use of ‘+’`というエラーメッセージとなっております。
...
下記のように書けば、`MaybeT IO`という「入出力処理を実行した後、結果が`Just`か`Nothing`かをチェックする」文脈を持った`Monad`型クラスのインスタンスの型が生まれます。
...
`runMaybeT $ (+) <$> MaybeT (neverZero 0) <*> MaybeT (neverZero 2)`という行に注目してください。  
...
つまり、`MaybeT IO`の`<*>`は、左辺に渡した`MaybeT IO (a -> b)`の文脈を実行して結果が`Nothing`だったら右辺の`MaybeT IO a`の`IO`の文脈から実行しない、という実装になっているのです。便利。
...
具体的には、`Monad`のインスタンスである型`m`は、任意の値`x :: a`と、該当の型の任意の値を返す関数`f :: a -> m b`、それから該当の型の任意の値`m`について、次の二組の式が**常に同じ意味**になるよう、`return`と`>>=`を実装しなければならない、というルールです。
...
`return x >>= f`と`f x`については、ラムダ式を挟んで、`return x >>= (\x -> f x)`と`(\x -> f x)`に書き換えてみると、より`>>=`と`return`を削除した形であることが分かりやすくなるでしょう。
...
`Maybe`型における`return`、すなわち「`>>=`や`do`記法における各行の間」で「`Just`か`Nothing`か確認しなければならない」という文脈において、「何もしないで値を返す処理」とは果たしてどんな処理でしょうか。  
これは試してみるのが一番の近道です。  
