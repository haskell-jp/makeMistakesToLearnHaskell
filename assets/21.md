...
こちらのような、`Maybe`型の値が`Just`か`Nothing`かを確認して、`Just`だったら関数を適用しつつ、`Nothing`だったら何もしないでそのまま`Nothing`を返す、といった処理はとてもありふれています。Haskellでは「関数を受け取る関数」も作れるので、例えば↓のような関数を定義することで、そうした処理を簡単に再利用可能なパーツにすることができます:
