...
`258`と出ました（通貨単位は日本円でしょうかね）！ここでは、GHCiに入力した結果を目視で確認して転記しました。一方、プログラムにこの一連の処理を行ってもらう場合、`lookup`関数で取得した結果が`Just`なのか`Nothing`なのか都度確認して、`Nothing`であれば何もしない[^error]、という確認をしなければなりません。このように、hoge
