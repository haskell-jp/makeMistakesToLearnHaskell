...
main = do
  ...
  case args of
      [name] -> do
        let eResult = formatResult
              <$> note "Fruit not found. He/She might like vegitables better." (M.lookup name fruitDictionary)
              <*> note "Birthday not found. He/She might want to hide his/her age." (M.lookup name birthdayDictionary)
              <*> note "Mail address not found. He/She might be afraid of spams." (M.lookup name mailAddressDictionary)
              <*> note "Prefecture not found. Where does he/she live?" (M.lookup name prefectureDictionary)
        case eResult of
