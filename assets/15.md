...
まずは「純粋な関数」や「命令」にとって必要不可欠な「値」について軽く触れておきます。「値」は関数が返す結果や、引数となります。「値」には「純粋な関数」や「命令」も含まれているのですが、どのような値であれ、それだけでは何もできません。詳細は後述しますが「純粋な関数」や「命令」が何かするには値との協力があってこそなのです。
...
値を受け取って返すだけ。数学における「関数」に近い。「関数型プログラミング」における「関数」とはこれのこと。値を受け取って返す以外に何もしない
