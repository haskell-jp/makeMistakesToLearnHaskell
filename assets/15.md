...
- 純粋な関数: 値を受け取って返すだけ。数学における「関数」に近い。「関数型プログラミング」における「関数」とはこれのこと。
    ...
    - `toUpper`
    - `+`, `*` などの四則演算に使う演算子
- ただの値: 純粋な関数が返した結果となる値。
    - `"hello"`
    - `True`
- 命令: 何かしら入出力処理を実行した上で、結果となる値を返したり返さなかったりするもの。一般的には「アクション」と呼ばれることの方が多いかも。
    - ほかのプログラミング言語で言うと、「引数をとらない関数オブジェクト」が近い。
        - Javaでいうところの`Callable`, Rubyでいうところの`Proc`, JavaScriptでいうところの`Function`, etc.
        - Haskell以外の言語は、普通の関数の中で入出力処理ができるので。
    - `IO 結果となる値の型`という型で表される。
...
ghci> (first : left) = actions
ghci> :t first
first :: IO ()
ghci> first
Hello, Everyone!

ghci> (first2 : left2) = left
ghci> first2
How are you?

ghci> (first3 : left3) = left2
ghci> first3
I'm fine!
```
...
「命令」の中には、`getLine`や`readFile`のように、「結果として返す値がある命令」があります。
...
- `map`は純粋な関数（ただの値を受け取ってただの値を返す関数）をリストの各要素に適用し、純粋な関数が返した値を、戻り値のリストに入れる。
- `for`は「結果を返す命令」を返す関数（ただの値を受け取って命令を返す関数）をリストの各要素に適用し、命令が返した値を、戻り値のリストに入れる。
- `for_`は「結果を返さない命令」を返す関数（ただの値を受け取って命令を返す関数）をリストの各要素に適用し、何も返さない（`IO ()`を返す）。
