...
`stringSoFar`という引数が増えましたね！ここに「これまで結合してきた文字列」が代入されます。`concatLines`関数は`getLine`関数を呼ぶ度に、この`stringSoFar`と`getLine`の結果`line`を結合します。そして、`line`が空文字列でなかった場合は、結合してできた文字列`newString`を新しい「これまで結合してきた文字列」として`concatLines`に渡し、再帰呼び出しします。
