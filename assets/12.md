...
`undefined`を使った場合と比べて、`*** Exception:`と書かれた後のメッセージが変わりましたね！このように、`error`関数は自分で設定したエラーメッセージの例外を発生させるのにとてもお手軽な機能なのです。

GHCiで`error`関数の型も見てみましょう。どんな型になっているか、`undefined`の型やこれまでに学習した知識から推測した上で入力してみてください。
